...
/**
 * components/admin/MissionPrincipleForm.tsx
 * ─────────────────────────────────────────────────────────────────────────
 * One "how we work" card on the About page. Doubles as the "add" form and
 * the inline editor for an existing one — same arrangement as
 * WhyUsPointForm (the two models are structurally identical, see
 * MissionPrinciple's schema.prisma comment for why they stay separate).
 *
 ...
 * page owns the single language selector shared by every principle's form
 * on the page — see AwardForm's file comment for the fuller version of
 * this note.
 * ─────────────────────────────────────────────────────────────────────────
 */
...
import type { MissionPrincipleFormState } from "@/app/[locale]/admin/(content)/pages/about/mission/actions";

export type MissionPrincipleValues = {
  icon: string;
  title: string;
  body: string;
  isActive: boolean;
...
export const EMPTY_MISSION_PRINCIPLE: MissionPrincipleValues = {
  icon: sectionIcons[0],
  title: "",
  body: "",
  isActive: true,
...
type Props = {
  lang: Locale;
  action: (
    state: MissionPrincipleFormState,
    formData: FormData,
  ) => Promise<MissionPrincipleFormState>;
  onDelete?: () => Promise<void>;
  values?: MissionPrincipleValues;
  submitLabel: string;
};

const INITIAL: MissionPrincipleFormState = { ok: false };
...
export default function MissionPrincipleForm({
  lang,
  action,
  onDelete,
  values = EMPTY_MISSION_PRINCIPLE,
  submitLabel,
}: Props) {
  ...
  return (
    <>
      <form action={formAction} className="space-y-5">
        ...
        <div>
          <label className="admin-label">{`${t("mission.pointTitle")} · ${lang.toUpperCase()}`}</label>
          <input
            name="title"
        ...
        <div>
          <label className="admin-label">{`${t("mission.body")} · ${lang.toUpperCase()}`}</label>
          <textarea
            name="body"
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <div>
            <label className="admin-label">{t("mission.icon")}</label>
          ...
          <div>
            <label className="admin-label">{t("mission.sortOrder")}</label>
            <input
              name="sortOrder"
              type="number"
