...
async function run() {
  const client = new S3Client({
    region: "ap-southeast-1", // Test ap-southeast-1 with .storage ? No, I need to test us-east-1 with .storage
    credentials: { 
  ...
  const cmd = new PutObjectCommand({ Bucket: "andamanassets", Key: "test.txt", ContentType: "text/plain" });
