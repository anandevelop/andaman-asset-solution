...
> **สถานะ: implement แล้ว ครบทุกหัวข้อ (2026-09-25)**
>
> เอกสารนี้เป็นผลจากการ audit โค้ดจริงใน repo ณ วันที่ 2026-09-10 และเก็บไว้เป็น
> บันทึกเหตุผล ไม่ใช่แผนที่ยังรออยู่ ตัวเลขในเอกสารเป็นเป้าตอนเสนอ —
> ผลจริงต่ำกว่าที่ตั้งไว้ เพราะ `docs/ADMIN_UX_CLEANUP_PROMPTS.md` เดินต่ออีกรอบ
> และยุบจาก 19 เหลือ **14** (ดูตาราง §2.2) หน้าแรกแบบแผงเดียวทำแล้วด้วย —
> ในรูปที่ต่างจากที่เสนอไว้ และ `docs/ADMIN_HOME_BUILDER_PLAN.md` §5 บันทึกว่าทำไม
>
> **แหล่งความจริงของโครงสร้างเมนูวันนี้คือ `lib/admin/nav.ts`** ไม่ใช่ §4.2 ของ
> เอกสารนี้ — โค้ดในนั้นเขียนไว้ตอนเสนอและต่างจากของจริงหลายจุด
> ส่วนพฤติกรรมที่ต้องคงไว้ถูกล็อกด้วยเทสใน `tests/admin/`
> (`nav.test.ts` มีตารางเมนูต่อ role, `permissions-nav.test.ts` เทียบเมนูกับ guard
> ของหน้าปลายทาง, `moved-screens.test.ts` และ `project-workspace.test.ts`
> ล็อกสิทธิ์เขียนกับ path ของหน้าที่ย้าย)
>
...
| Role | ก่อน | เป้าตอนเสนอ | **ของจริงวันนี้** | หมายเหตุ |
|---|---|---|---|---|
| SUPER_ADMIN | 26 | 18 | **14** | −46% |
| ADMIN | 26 | 18 | **12** | ไม่มี users / activity (OWNER_ONLY) |
| EDITOR | 20 | 11 | **8** | ไม่มีลิงก์หลอกอีกต่อไป (แก้ P2) |
| SALES | 5 | 5 | **3** | เห็น Sales Team ได้แล้ว (แก้ P5) |
| VIEWER | 1 | 10 | **7** | อ่านอย่างเดียว — ทำให้ `viewContent` มีความหมาย (แก้ P3/P4) |

นับเฉพาะ rail บนเดสก์ท็อป: "โหมดมือถือ" เป็น `mobileOnly` ขึ้นเฉพาะใน drawer
ตัวเลขชุดนี้ถูกยืนยันด้วยตารางใน `tests/admin/nav.test.ts` ไม่ใช่การนับด้วยมือ
