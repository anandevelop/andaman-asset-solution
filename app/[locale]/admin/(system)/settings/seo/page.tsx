/**
 * app/[locale]/admin/settings/seo/page.tsx
 * ─────────────────────────────────────────────────────────────────────────
 * Branding and SEO — how the site looks in a search result, a share
 * preview and a browser tab.
 *
 * Split out of /admin/settings rather than appended to it: that page is
 * where someone goes to fix a phone number, and burying it under thirty
 * more fields would make the common task the hard one.
 *
 * It binds the *same* updateSettings action. That works because the action
 * iterates SETTING_KEYS and skips any key the submitted form did not carry,
 * so each page writes only its own subset and neither can wipe the other's
 * values. See the note at the top of ../actions.ts.
 *
 * ADMIN and above, matching the parent page: a meta title is the company's
 * public identity in exactly the way a phone number is.
 * ─────────────────────────────────────────────────────────────────────────
 */
...
export default async function AdminSeoSettingsPage(props: Props) {
