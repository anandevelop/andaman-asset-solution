 ...
 * WHAT IS STORED
 *
 * One row per path per day with a number on it. No IP, no identifier, no
 * session, nothing that could reconstruct one person's reading history —
 * the admin screen asks "how many", so "how many" is all that is kept.
 * ─────────────────────────────────────────────────────────────────────────
 */
...
/**
 * Only paths this screen reports on are counted.
 *
 * Not a security measure — it is what keeps the table from filling up with
 * one row per day for every URL a crawler invents. Anything not on the
 * list is accepted and ignored, so a page that starts sending beacons
 * before this list knows about it fails quietly rather than 400ing in
 * somebody's console.
 */
const COUNTED_PREFIXES = ["/news/"];
...
export async function POST(request: Request) {
  ...
  if (!COUNTED_PREFIXES.some((prefix) => path.startsWith(prefix))) {
    return new NextResponse(null, { status: 204 });
  }
